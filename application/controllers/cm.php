...
//
	...
	/** 
		start other data source 3-08-2017
	**/
	
	public function rintaothers() {
        $title = $_POST['title'];
        //BreadCrumb
        ...
        $this->load->view('channel_mgm/rintaothers',$result);
    }
	
	public function viewMycoins() {
        $data['period'] = $this->input->post('tahun')."".$this->input->post('bulan');
        $data['pgl_id'] = $this->input->post('pengelola');
        $data['dtsc_id'] = 31;
        $this->load->view('channel_mgm/detail_rinta_others',$data);
    }
	
	
	
	public function gridMyCoins($period,$pgl_id,$dtsc_id) {
        $sidx = $_POST['sidx'] ;
        $sord = $_POST['sord'] ;
        ...
        $row = $this->cm->getMycoinsCount($req_param);
        $count= $row[0]->COUNT;
        ...
        $result['Data'] = $this->cm->getMycoins($req_param)->result_array();
	...
	/** 
		end other data source 3-08-2017
	**/
	...
	public function viewIdeas() {
        ...
        $data['pgl_id'] = $this->input->post('pengelola');
        $data['dtsc_id'] = 32;
        $this->load->view('channel_mgm/detail_rinta_others_ideas',$data);
    }
	
	public function gridIdeas($period,$pgl_id,$dtsc_id) {
        $sidx = $_POST['sidx'] ;
        $sord = $_POST['sord'] ;
        $page = intval($_REQUEST['page']) ;
        $limit = $_REQUEST['rows'] ;
        ...
        $row = $this->cm->getIdeasCount($req_param);
        $count= $row[0]->COUNT;
        ...
        $result['Data'] = $this->cm->getIdeas($req_param)->result_array();
	...
	public function tunggakanDTSheet($pgl_id, $ten_id, $period) {
	   ...
	   $sh->getStyle('R1')->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	    ...
	    $sh->getStyle('P1:P'.$x)->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	    $sh->getStyle('Q1:Q'.$x)->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	    $sh->getStyle('R1:R'.$x)->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
		$x++;
        $sh->setCellValue('B'.$x, 'TOTAL');     
        ...
        $sh->getStyle('O'.$x)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $sh->getStyle('P'.$x)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $sh->getStyle('Q'.$x)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        // $sh->getStyle('M'.$x)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        // $sh->getStyle('L'.$x)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objWriter = PHPExcel_IOFactory::createWriter($this->phpexcel, 'Excel5');
        $objWriter->save(dirname(__FILE__).'/../third_party/report/'.$filename);
	...
	/*DTS MY COIN 3-13 by RND*/
	  // Sheet Output
	  public function rintasheetIdeas($pgl_id, $dtsc_id, $period) {
        // Set unlimited usage memory for big data
        ini_set('memory_limit', '-1');
        // Sheet
        $this->load->library("phpexcel");
        $filename = "rinta_ID3AS_".$pgl_id."_".$dtsc_id."_".$period.".xls";
        $this->phpexcel->getProperties()->setCreator("PT Telekomunikasi Indonesia, Tbk")
            ->setLastModifiedBy("PT Telekomunikasi Indonesia, Tbk")
            ->setTitle("REPORT")
            ->setKeywords("office 2007 openxml php")
            ->setCategory("Rincian Tagihan MY-COIN");
        $this->phpexcel->setActiveSheetIndex(0);
        $sh = & $this->phpexcel->getActiveSheet();
        $sh->setCellValue('A1', 'ND')
            ->setCellValue('B1', 'NAMA PLG')
        ...
        $x = 2;
		/*model query*/
        $dt = $this->cm->excelRintaIdeas($period, $pgl_id, $dtsc_id);
        $no = 1;
        foreach($dt as $k => $r) {
            $sh->getCell('A'.$x)->setValueExplicit($r->ND, PHPExcel_Cell_DataType::TYPE_STRING);
            $sh->setCellValue('B'.$x, @$r->NAMA);
            $sh->setCellValue('C'.$x, @$r->ABONEMEN);
            $sh->setCellValue('D'.$x, @$r->MNT_TCK_C);
            $sh->setCellValue('E'.$x, @$r->MNT_TCK_D);
            $sh->setCellValue('F'.$x, @$r->LOKAL);
    ...
    public function rintasheetCoin($pgl_id, $dtsc_id, $period) {
        // Set unlimited usage memory for big data
        ini_set('memory_limit', '-1');
        // Sheet
        $this->load->library("phpexcel");
        $filename = "rinta_MyCoin_".$pgl_id."_".$dtsc_id."_".$period.".xls";
        $this->phpexcel->getProperties()->setCreator("PT Telekomunikasi Indonesia, Tbk")
            ->setLastModifiedBy("PT Telekomunikasi Indonesia, Tbk")
            ->setTitle("REPORT")
            ->setKeywords("office 2007 openxml php")
        ...
        $x = 2;
		/*model query*/
        $dt = $this->cm->excelRintaCoin($period, $pgl_id, $dtsc_id);
        $no = 1;
        foreach($dt as $k => $r) {
            $sh->getCell('A'.$x)->setValueExplicit($r->ND, PHPExcel_Cell_DataType::TYPE_STRING);
            $sh->setCellValue('B'.$x, @$r->NAMA);
	...
	public function fastelsheet($pgl_id, $period) {
        ...
        foreach($dt as $k => $r) {
            ...
            $sh->setCellValue('L'.$x, @$r->PROD_PERIOD);
            $no++;
            $x++;
            //if($x==8000) break;
        }
